...
class Pages extends CI_Controller {
	...
	public function index()
	{
		...
		$config['total_rows'] = $this->Pagix_model->countPages();
		$config['per_page'] = $result_per_page;
		$this->pagination->initialize($config);
		//Get pages from database
		$this->data['pages'] = $this->Pagix_model->getPages($result_per_page, $this->uri->segment(3));
	...
	public function confirm()
	{
		...
		$this->form_validation->set_rules('pageURL', 'page URL', 'trim|alpha_dash|required|is_unique[pagix_page_attributes.pageURL]');
	...
	public function editPage()
	{
		...
		$this->data['pages'] = $this->Pagix_model->getPage($this->uri->segment(4));
		//Load the view
		$this->data['templates'] = get_filenames('theme/'.THEME.'/templates');
		$this->data['header'] = $this->load->view('admin/header', $this->data, true);
		$this->data['footer'] = $this->load->view('admin/footer', '', true);
	...
	public function edited()
	{
		...
		$this->form_validation->set_rules('pageURL', 'page URL', 'trim|alpha_dash|required|is_unique[pagix_page_attributes.pageURL.pageID.'.$this->uri->segment(4).']');
		}
		$this->form_validation->set_rules('pageTitle', 'page title', 'trim|required');
		$this->form_validation->set_rules('navTitle', 'navigation title', 'trim|required');
		...
		if($this->form_validation->run() == FALSE) {
			...
			//Update the page
			$this->load->library('Sioen');
			$this->Pagix_model->updatePage($this->uri->segment(4));
			//Return to page list
			redirect(BASE_URL.'/admin/pages', 'refresh');
	  	}
	}
	...
	public function jumbo()
	{
		//Get page details from database
		$this->data['pages'] = $this->Pagix_model->getPage($this->uri->segment(4));
		$this->data['slides'] = $this->Pagix_model->getPageBanners($this->uri->segment(4));
	...
	{
		$this->load->library('Sioen');
		$this->Pagix_model->updateJumbotron($this->uri->segment(4));
		redirect(BASE_URL.'/admin/pages', 'refresh');
	}
	...
	function delete()
	{
		if($this->input->post('deleteid')):
			$this->Pagix_model->removePage($this->input->post('deleteid'));
			redirect('/admin/pages');
		else:
			$this->data['form']=$this->Pagix_model->getPage($this->uri->segment(4));
	...
	function pageSearch()
	{
		$this->Pagix_model->pageSearch($this->input->post('term'));
	}
