...
class Users extends CI_Controller {
    ...
    $config['total_rows'] = $this->Pagix_model->countUsers();
    $config['per_page'] = $result_per_page;

    $this->pagination->initialize($config);

		//Get users from database
		$this->data['users'] = $this->Pagix_model->getUsers($result_per_page, $this->uri->segment(3));
	...
	public function confirm()
	{
		...
		$this->form_validation->set_rules('username', 'username', 'trim|alpha_dash|required|is_unique[pagix_user.userName]');
		$this->form_validation->set_rules('email', 'email address', 'trim|required|valid_email|is_unique[pagix_user.email]');
	...
	public function editUser()
	{
		...
		$this->data['users'] = $this->Pagix_model->getUser($this->uri->segment(4));
		//Load the view
		$this->data['header'] = $this->load->view('admin/header', $this->data, true);
		$this->data['footer'] = $this->load->view('admin/footer', '', true);
	...
	public function edited()
	{
		...
		$this->load->library('form_validation');
		//Set validation rules
		$this->form_validation->set_rules('email', 'email address', 'trim|required|valid_email|is_unique[pagix_user.email.userID.'.$this->uri->segment(4).']');
		$this->form_validation->set_rules('password', 'password', 'trim|required|min_length[4]|max_length[32]');
		$this->form_validation->set_rules('con_password', 'confirm password','trim|required|matches[password]');
		...
		if($this->form_validation->run() == FALSE) {
			...
			//Validation passed
			//Update the user
			$this->Pagix_model->updateUser($this->uri->segment(4));
			//Return to user list
			redirect(BASE_URL.'/admin/users', 'refresh');
	  	}
	}
		...
		function delete()
	{
		...
		if($this->input->post('deleteid')):
			$this->Pagix_model->removeUser($this->input->post('deleteid'));
			redirect(BASE_URL.'/admin/users');
		else:
			$this->data['form']=$this->Pagix_model->getUser($this->uri->segment(4));
	...
	{
		$query = $this->db->get_where('pagix_user', array('email' => $str), 1);
		if ($query->num_rows()== 1)
		{
			return true;
		}
	...
	public function getPassword()
	{
		...
		$query=$this->db->get_where('pagix_user', array('rs' => $rs), 1);
